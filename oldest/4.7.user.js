...
notifyUser = {
  ...
  addAbFilter : function (filters) {
    ...
    this.box('<u>Add filter(s) on your Ad Blocker</u> <br/><code> ' + filters + '</code> ', 8000);
  }
};
...
Rules = {
  ...
  kissanime : {
  	...
  	Loaded : function () {
  		...
  		if (/kissanime.com\/Anime\/.+\/(OVA|Episode)-.+/i.test(location.href)) {
  			...
  			// Exclude Fake Player
  			var players = [];
  			for (var v = 0; v < embeds.length; v++) {
  				if (embeds[v].parentNode.style.display != "none" && 
				embeds[v].parentNode.style.visibility != "hidden" && 
				embeds[v].style && 
				embeds[v].style.display != "none" && 
				embeds[v].style.visibility != "hidden") {
  					players.push(embeds[v]);
  				}
  			}
  ...
  tvdez : { 
    // (document.getElementById('pubfooter').clientHeight < 20)
  	Host : ['tvdez.com','casadossegredos.tv','estadiofutebol.com'],
    Inject : function () {
	  //setCookie('adblock',null);
	  addElement('pubfooter');
      GM_addStyle("#pubfooter { height: 30px !important; display: block !important; }");
    },	
  	BeforeFix : function () {
  		notifyUser.addAbFilter("tvdez.com,casadossegredos.tv,estadiofutebol.com#@##pubfooter");
  	},
  	Before : function (e) {
