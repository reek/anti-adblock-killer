...
Coding helpful:
  InfinityCoding, Couchy, Dindog, Floxflob, U Bless. Watilin
  
Donors:
  Mike Howard, Shunjou, Charmine, Kierek93, George Barnard, Henry Young, Seinhor9
...
function AakPlayer(file, width, height) {
	...
	player.setAttribute("flashvars", "file=" + file);
    return player;
}
...
Rules = {
  ...
  videofun : {
    ...
    Inserted : function (insertedNode) { // Allow video fullscreen
      if (insertedNode.id && insertedNode.id == 'flowplayer_api') {
  ...
  playtv : {
    ...
    Loaded : function () {
      ...
      */
      ...
      if (/playtv.fr/.test(location.host)) {
        location.host = 'm.playtv.fr';
      }
    }
  },
  myspass : {
    ...
    Loaded : function () {
  	...
  	Inserted : function (insertedNode) {
  		...
  		if (insertedNode.id &&
  			...
  			abId.test(insertedNode.id) &&
			abNodeName1.test(insertedNode.nodeName) &&					
			abNodeName2.test(insertedNode.firstChild.nodeName)) {
  			//console.log(insertedNode);

  			// Disable versions 2
  			if (abWords1.test(insertedNode.innerHTML) &&
  				abWords2.test(insertedNode.innerHTML)) {
  				removeElement(insertedNode);
				//console.log(insertedNode);
  			}
  			// Disable versions 3
			    ...
			    insertedNode.firstChild.firstChild.nodeName == "IMG" &&
  				abImg.test(insertedNode.firstChild.firstChild.src)) {
  				removeElement(insertedNode);
				//console.log(insertedNode);
  			}
